...
namespace lightning::loc
{
    ...
    bool Localization::Init(const std::string &yaml_path, const std::string &global_map_path)
    {
        ...
        lidar_loc_proc_cloud_.SetProcFunc([this](CloudPtr cloud)
                                          { LidarLocProcCloud(cloud); });
        ...
        if (odom_callback_ && loc_result_.valid_) {
            odom_callback_(loc_result_);
        }
    ...
    void Localization::SetOdomCallback(OdomCallback &&callback)
    {
